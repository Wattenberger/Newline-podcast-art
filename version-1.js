...
const data = {
  ...
  imageSize: +getLocalStorage("imageSize") || 1,
  imageOffsetX: +getLocalStorage("imageOffsetX") || 0,
  imageOffsetY: +getLocalStorage("imageOffsetY") || 0,
      ...
      try {
        ...
        image.onload = () => {
          const duotoneImage = Filters.duotone(
            ...
            data.filter,
            data.imageSize,
            [data.imageOffsetX, data.imageOffsetY]
          )
          context.globalCompositeOperation = "normal"
          context.globalAlpha = 0.6
          context.drawImage( duotoneImage, 0, 0 )
          context.globalAlpha = 1
...
(async () => {
  ...
  if (useGUI) {
    ...
    add(gui, data, "imageSize", 0, 2, 0.03).name("image size")
    add(gui, data, "imageOffsetX", -0.5, 0.5, 0.01).name("image offset x")
    add(gui, data, "imageOffsetY", -0.5, 0.5, 0.01).name("image offset y")
    add(gui, data, "filter")
    add(gui, data, "guest").name("guest name")
    add(gui, data, "imageName").name("image filename")
    add(gui, data, "mode", modes).name("image blend mode")
    add(gui, data, "circleMode", modes).name("circle blend mode")
...
let Filters = {}
Filters.getPixels = function(ctx, image, width, height, filter, imageSize, imageOffsets) {
  ...
  context2.drawImage(
    ...
    width * imageOffsets[0], width * imageOffsets[1],
...
Filters.duotone = function(ctx, img, width, height, tone1, tone2, filter, imageSize, imageOffsets) {
  var [
    bufferCanvas,
    context2,
    pixels,
  ] = this.getPixels(ctx, img, width, height, filter, imageSize, imageOffsets);
